...
export default class MlNav {
  ...
  switcher (device, top) {
    ...
    const styles: {[elclass:string]: { [property: string]: string | undefined }} = {
      'dropdown': {
        'position': 'absolute',
        'top': `${top - 20}px`,
        'right': device === 'desktop' ? '2rem' : undefined,
        'left': device !== 'desktop' ? '2rem' : undefined,
      ...
      'dropdown-content': {
        'right': device === 'desktop' ? '10px' : undefined,
        'top': '3rem',
        'display': 'none',
        'position': 'absolute',
  ...
  renderNav () {
    ...
    // deskop switched
    const desktopNav = document.querySelector('.header-display-desktop') as HTMLElement
    if (desktopNav){
      render(this.switcher('desktop', desktopNav?.clientHeight/2), desktopNav)
    }
    const mobileNav = document.querySelector('.header-display-mobile') as HTMLElement
    if( mobileNav) {
      render(this.switcher('mobile', mobileNav.clientHeight/2), mobileNav)
    }
